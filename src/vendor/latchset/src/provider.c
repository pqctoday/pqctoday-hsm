...
#define PQC_MECHS \
    ...
    CKM_SLH_DSA, CKM_SLH_DSA_KEY_PAIR_GEN, CKM_XMSS, CKM_XMSSMT, CKM_HSS
    ...
    for (slot = p11prov_fetch_slot(slots, &slot_idx); slot != NULL;
        ...
        for (int ms = 0; ms < nmechs; ms++) {
            ...
            case CKM_SLH_DSA_KEY_PAIR_GEN:
                ...
                break;
            case CKM_XMSS:
                ADD_ALGO_EXT(XMSS, signature, prop, p11prov_xmss_signature_functions);
                UNCHECK_MECHS(CKM_XMSS);
                break;
            case CKM_XMSSMT:
                ADD_ALGO_EXT(XMSSMT, signature, prop, p11prov_xmssmt_signature_functions);
                UNCHECK_MECHS(CKM_XMSSMT);
                break;
            case CKM_HSS:
                ADD_ALGO_EXT(HSS, signature, prop, p11prov_hss_signature_functions);
                ADD_ALGO_EXT(LMS, signature, prop, p11prov_lms_signature_functions);
                UNCHECK_MECHS(CKM_HSS);
                break;
            case CKM_ML_KEM:
